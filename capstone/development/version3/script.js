(function(){
    ...
    const sighLine = document.getElementById('sigh-line');
    const pastSad = document.getElementById('past-sad');
    const pastSigh = document.getElementById('past-sigh');
    const happyLine = document.getElementById('happy-line');
    const presentSad = document.getElementById('present-sad');
    const presentHappy = document.getElementById('present-happy');
    ...
    const pastObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                pastSad.style.display = 'none';
                pastSigh.style.display = 'block';
            } else {
                pastSad.style.display = 'block';
                pastSigh.style.display = 'none';
            }
        });
    }, { threshold: 0.6 });

    if (sighLine) {
        pastObserver.observe(sighLine);
    }

    const presentObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                presentSad.style.display = 'none';
                presentHappy.style.display = 'block';
            } else {
                presentSad.style.display = 'block';
                presentHappy.style.display = 'none';
            }
        });
    }, { threshold: 0.6 });

    if (happyLine) {
        presentObserver.observe(happyLine);
    }
